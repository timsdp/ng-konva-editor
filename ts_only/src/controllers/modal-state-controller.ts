class ModalStateController {
    private containerElementId = "modalState";
    private bodyElementId = "modalStateBody";
    private headerElementId = "modalStateHeader";
    private buttonImportElementId = "btnStateModalImport";
    private stateExamples: string[] = [
        `[
            {
                "Sizable": false,
                "X": 20,
    ...
    public Show(caption: string, content: string){
    ...
    private onImportButtonClick(event: any){

        let inner = event.data;
        let json = $(`#${inner.bodyElementId}`).html();
        let model: Control[] = JSON.parse(json);
